...
def test_reorder_from_graph_batches_roundtrip():
    ...
    x_graphs, idx_out, mask_out = pack_to_graph_batches(x_original, [], batch)
    x_recovered = reorder_from_graph_batches(x_graphs[0], idx_out)

    assert torch.allclose(
        x_recovered, x_original
    ), f"Roundtrip failed. Expected {x_original}, got {x_recovered}"
    assert (
        mask_out.shape[0] == x_graphs[0].shape[0]
